...
export class ShopifyAppService {
    ...
    static async registerAppUninstallWebhook(shopDomain, shopId) {
        try {
            ...
            const response = await axios.post(`https://${shopDomain}/admin/api/2024-01/graphql.json`, {
                query: `
            mutation {
    ...
    static async registerRefundsCreateWebhook(shopDomain, shopId) {
        try {
            ...
            if (!baseUrl || !baseUrl.startsWith('https://'))
                return;
            const refundRes = await axios.post(`https://${shopDomain}/admin/api/2024-01/graphql.json`, {
                query: `
            mutation {
              webhookSubscriptionCreate(
