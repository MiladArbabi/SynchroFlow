// tests/unit/backend/ft2/finances.ft2.controller.presetAuthority.test.ts
...
jest.mock(
  'api-src/services/finances-ft2.provider',
  () => ({
    getFinancesFt2Snapshot: (input: any) =>
...
// Import AFTER mocks
// ─────────────────────────────────────────────
import financesFt2Router from
  'api-src/api/finances/finances.ft2.routes';

// ─────────────────────────────────────────────
// Test
// ─────────────────────────────────────────────
describe(
  'Finances FT2 Controller — preset authority (RED)',
  () => {
    it(
      ...
      async () => {
        ...
        app.use(
          '/api/v1/modules/finances',
          financesFt2Router as unknown as express.Router
        );
