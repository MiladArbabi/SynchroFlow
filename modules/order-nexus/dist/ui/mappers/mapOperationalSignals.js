...
import { registerSignalType, getDetectedAt } from './lifecycle/signalLifecycleEngine.js';
 ...
 */
{
    ...
    if (duplicates.length > 0) {
        console.error('[OperationalSignals] Duplicate signal identifiers detected:', duplicates);
    }
}
/**
...
export function mapOperationalSignals(snapshot, currency) {
    const safeSnapshot = normalizeOperationalSnapshot(snapshot);
    /**
     ...
     */
    const evaluationTime = new Date(snapshot.snapshot_date).getTime();
    if (Number.isNaN(evaluationTime)) {
        console.error('[OperationalSignals] Invalid snapshot_date in operational snapshot', snapshot.snapshot_date);
    }
    /**
     ...
     */
    if (evaluationTime === 0) {
        console.warn('[OperationalSignals] evaluationTime missing from snapshot — using deterministic fallback');
    }
    /**
    ...
    validateSnapshotCoverage(snapshot, SNAPSHOT_FIELD_COVERAGE);
    const signals = [];
    /**
     ...
     */
    const activeSignalTypes = new Set();
    /**
    ...
    for (const entry of signalRegistry) {
        ...
        const detectedAt = getDetectedAt(entry.id, evaluationTime);
        if (registerSignalType(activeSignalTypes, entry.id)) {
            signals.push(entry.build({
...
//# sourceMappingURL=mapOperationalSignals.js.map