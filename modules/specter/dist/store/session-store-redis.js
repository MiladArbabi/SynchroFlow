...
export class RedisSessionStore {
    ...
    snapshot = new Map();
    listMaxLen = Number(process.env.SPECTER_SESSION_STORE_LIST_MAX || 1000);
    // Events snapshot + max length (new)
    eventsSnapshot = new Map(); // SpecterEvent[] (avoid import cycle in this file)
    eventListMaxLen = Number(process.env.SPECTER_EVENT_LIST_MAX || 50);
    redisUrl = process.env.SPECTER_REDIS_URL || process.env.REDIS_URL || 'redis://localhost:6379';
    constructor(opts) {
        if (opts?.redisUrl)
    ...
    async saveSession(session) {
        ...
        if (!toSave.sessionId)
            toSave.sessionId = (randomUUID && typeof randomUUID === 'function') ? randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
...
export async function initRedisSessionStore(store) {
    const s = store || new RedisSessionStore();
    await s.init();
    return s;
}
export async function closeRedisSessionStore(store) {
    if (store)
        await store.close();
}
