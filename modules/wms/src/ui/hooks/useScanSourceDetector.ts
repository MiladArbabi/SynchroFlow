...
import type { ConfirmScanParams } from '../pages/PickSessionPage';
...
export function useScanSourceDetector() {
  ...
  const detectSourceAndReset = useCallback((): ConfirmScanParams['scan_source'] => {
    const start = firstKeystrokeAt.current;
    firstKeystrokeAt.current = null; // reset for next scan

    if (start === null) return 'manual';
