...
// ── CanvasEditor ──────────────────────────────────────────────────────────────
export function CanvasEditor({ zones, onUpdateZone, onDeleteZone, onCreateZone, onPrintBarcode, }) {
        ...
        async function onMouseUp() {
            ...
            if (marqueeStartRef.current && marqueeRef.current) {
                ...
                if (maxX - minX > 4 || maxY - minY > 4) {
                    ...
                    if (hits.length >= 1)
                        setSelected(hits[0].location_code);
    ...
    return (_jsxs(Box, { sx: { display: 'flex', width: '100%', height: 560, border: '1px solid var(--rule)', borderRadius: 2, overflow: 'hidden', bgcolor: 'var(--bg)' }, children: [renderMode === '2D' && _jsx(ComponentPalette, { zones: zones, unpositionedZones: zones.filter(z => z.position_x == null), onPlace: (zone) => {
                    ...
                    onUpdateZone?.(zone.location_code, { position_x: centreX, position_y: centreY });
                                    ...
                                    { m: 'select', icon: _jsx(MousePointer, { size: 12 }), title: 'Select mode — click or drag to select zones' },
                                        ...
                                        transition: 'all 0.15s' }, children: icon }, m))) }), renderMode === '2D' && _jsxs(Box, { sx: { ml: 'auto', display: 'flex', alignItems: 'center', gap: 0.5 }, children: [[{ label: '−', delta: -0.15 }, { label: '+', delta: 0.15 }].map(({ label, delta }) => (_jsx(Box, { onClick: () => setZoom(z => Math.min(MAX_ZOOM, Math.max(MIN_ZOOM, z + delta))), sx: { width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--rule)', borderRadius: 1, cursor: 'pointer', fontSize: 13, color: 'var(--ink-3)', bgcolor: 'var(--bg)', '&:hover': { borderColor: 'var(--accent)', color: 'var(--accent)' } }, children: label }, label))), _jsxs(Box, { sx: { px: 1, height: 22, display: 'flex', alignItems: 'center', border: '1px solid var(--rule)', borderRadius: 1, fontSize: 10, color: 'var(--ink-3)', bgcolor: 'var(--bg)', fontFamily: 'monospace', minWidth: 40, justifyContent: 'center' }, children: [Math.round(zoom * 100), "%"] }), _jsx(Box, { onClick: () => { setZoom(1); setOffset({ x: 40, y: 40 }); }, sx: { px: 1, height: 22, display: 'flex', alignItems: 'center', border: '1px solid var(--rule)', borderRadius: 1, fontSize: 10, color: 'var(--ink-3)', bgcolor: 'var(--bg)', cursor: 'pointer', '&:hover': { borderColor: 'var(--accent)', color: 'var(--accent)' } }, children: "Reset" })] })] }), renderMode === '3D' && (_jsx(Box, { sx: { flex: 1, overflow: 'hidden' }, children: _jsx(IsometricCanvas, { zones: zones, onSelect: (code) => setSelected(code) }) })), renderMode === '2D' && _jsxs(Box, { sx: { flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }, children: [_jsxs(Box, { sx: { display: 'flex', flexShrink: 0 }, children: [_jsx(Box, { sx: { width: RULER_SIZE, height: RULER_SIZE, bgcolor: 'var(--bg-2)', borderRight: '1px solid var(--rule)', borderBottom: '1px solid var(--rule)', flexShrink: 0 } }), _jsx(Box, { sx: { flex: 1, overflow: 'hidden' }, children: _jsx(Ruler, { length: CANVAS_W, horizontal: true, scale: SCALE, offset: offset.x, zoom: zoom }) })] }), _jsxs(Box, { sx: { flex: 1, display: 'flex', overflow: 'hidden' }, children: [_jsx(Box, { sx: { width: RULER_SIZE, flexShrink: 0, overflow: 'hidden' }, children: _jsx(Ruler, { length: CANVAS_H, horizontal: false, scale: SCALE, offset: offset.y, zoom: zoom }) }), _jsxs(Box, { sx: { flex: 1, overflow: 'hidden', position: 'relative' }, children: [_jsxs("svg", { ref: svgRef, width: "100%", height: "100%", style: { cursor: mode === 'select' ? 'crosshair' : 'grab', userSelect: 'none', display: 'block' }, onMouseDown: onCanvasMouseDown, children: [_jsxs("g", { transform: `translate(${offset.x},${offset.y})`, children: [_jsx("defs", { children: _jsx("pattern", { id: "grid-dots", x: "0", y: "0", width: SCALE * zoom * SNAP, height: SCALE * zoom * SNAP, patternUnits: "userSpaceOnUse", children: _jsx("circle", { cx: "1", cy: "1", r: "1", fill: "var(--rule)", opacity: "0.5" }) }) }), _jsx("rect", { x: "0", y: "0", width: canvasW, height: canvasH, fill: "url(#grid-dots)" }), _jsx("rect", { x: "0", y: "0", width: canvasW, height: canvasH, fill: "none", stroke: "var(--rule)", strokeWidth: "1" }), [...positionedZones].sort((a, b) => {
                                            ...
                                            '&:hover': { color: 'var(--accent)' } }, children: z.location_code }), i < arr.length - 1 && _jsx(Typography, { sx: { fontSize: 10, color: 'var(--rule)', mx: 0.25 }, children: "/" })] }, z.location_code)))] })] }), selectedZone ? (_jsx(RackInspector, { zone: selectedZone, onClose: () => setSelected(null), onUpdateZone: renderMode === '2D' ? onUpdateZone : undefined, onDeleteZone: renderMode === '2D' ? onDeleteZone : undefined, onPrintBarcode: onPrintBarcode, onCreateZone: onCreateZone })) : (_jsxs(Box, { sx: { width: 220, flexShrink: 0, borderLeft: '1px solid var(--rule)', bgcolor: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1, p: 2 }, children: [_jsx(Box, { sx: { width: 32, height: 32, borderRadius: 1, border: '1.5px dashed var(--rule)', display: 'flex', alignItems: 'center', justifyContent: 'center' }, children: _jsx(Layers, { size: 14, color: "var(--ink-4)" }) }), _jsx(Typography, { sx: { fontSize: 11, color: 'var(--ink-4)', textAlign: 'center', lineHeight: 1.4 }, children: "Click a zone to inspect and edit" })] }))] }));
}
