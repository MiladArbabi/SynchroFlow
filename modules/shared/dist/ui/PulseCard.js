...
function PulseCardRow({ row }) {
    ...
    const content = (_jsxs(Box, { sx: {
        ...
        }, children: [_jsxs(Box, { sx: { display: 'flex', flexDirection: 'column', gap: 0.5, flex: 1, minWidth: 0 }, children: [_jsxs(Box, { sx: { display: 'flex', alignItems: 'center', gap: 0.75 }, children: [_jsx(Box, { "aria-hidden": true, sx: { width: 6, height: 6, borderRadius: '50%', bgcolor: TONE_COLOR[tone], flexShrink: 0 } }), _jsx(Typography, { sx: { fontSize: 13, fontWeight: 400, color: 'var(--ink-3)' }, children: row.label })] }), row.subtext && (_jsx(Typography, { sx: { fontSize: 11, fontWeight: 300, color: row.subtextTone ? TONE_COLOR[row.subtextTone] : 'var(--ink-4)', pl: '14px' }, children: row.subtext })), row.progress && (_jsx(Box, { sx: { height: 4, borderRadius: '2px', bgcolor: 'var(--rule)', overflow: 'hidden', ml: '14px', mt: 0.5 }, children: _jsx(Box, { sx: { height: '100%', width: `${Math.min(100, (row.progress.value / row.progress.max) * 100)}%`, bgcolor: TONE_COLOR[tone], transition: 'width 0.2s' } }) }))] }), row.action ? (_jsx(Box, { component: "button", onClick: (e) => { e.stopPropagation(); row.action.onClick(); }, sx: {
...
export function PulseCard({ title, headline, rows, footerCta, updatedAt, onRefresh, variant = 'card' }) {
