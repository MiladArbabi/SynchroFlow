...
function PulseCardRow({ row }) {
    ...
    const content = (_jsxs(Box, { sx: {
            ...
            '&:focus-visible': clickable ? { outline: '2px solid var(--accent)', outlineOffset: '2px' } : undefined,
        }, children: [_jsxs(Box, { sx: { display: 'flex', flexDirection: 'column', gap: 0.5, flex: 1, minWidth: 0 }, children: [_jsxs(Box, { sx: { display: 'flex', alignItems: 'center', gap: 0.75 }, children: [_jsx(Box, { "aria-hidden": true, sx: { width: 6, height: 6, borderRadius: '50%', bgcolor: row.colorOverride ?? TONE_COLOR[tone], flexShrink: 0 } }), _jsx(Typography, { sx: { fontSize: 13, fontWeight: 400, color: 'var(--ink-3)' }, children: row.label })] }), row.subtext && (_jsx(Typography, { sx: { fontSize: 11, fontWeight: 300, color: row.subtextTone ? TONE_COLOR[row.subtextTone] : 'var(--ink-4)', pl: '14px' }, children: row.subtext })), row.progress && (_jsx(Box, { sx: { height: 4, borderRadius: '2px', bgcolor: 'var(--rule)', overflow: 'hidden', ml: '14px', mt: 0.5 }, children: _jsx(Box, { sx: { height: '100%', width: `${Math.max(Math.min(100, (row.progress.value / row.progress.max) * 100), row.progress.value > 0 ? 8 : 0)}%`, bgcolor: row.colorOverride ?? TONE_COLOR[tone], transition: 'width 0.2s' } }) }))] }), row.action ? (_jsx(Box, { component: "button", onClick: (e) => { e.stopPropagation(); row.action.onClick(); }, sx: {
                ...
                }, children: row.action.label })) : (_jsx(Typography, { sx: { fontSize: 15, fontWeight: 600, color: row.colorOverride ?? (tone === 'neutral' ? 'var(--ink)' : TONE_COLOR[tone]), flexShrink: 0, pl: 1 }, children: row.value }))] }));
    if (!clickable)
        return content;
...
export function PulseCard({ title, headline, rows, footerNote, footerCta, updatedAt, onRefresh, variant = 'card' }) {
    ...
    return (_jsxs(Box, { sx: { ...shellSx, display: 'flex', flexDirection: 'column', gap: 1 }, children: [_jsx(Typography, { sx: { fontSize: 10, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--ink-4)' }, children: title }), headline && (_jsxs(Box, { sx: { pb: 1, mb: 0.5, borderBottom: '1px solid var(--rule)' }, children: [_jsx(Typography, { sx: { fontSize: 28, fontWeight: 600, color: headline.colorOverride ?? TONE_COLOR[headline.tone], lineHeight: 1.1 }, children: headline.value }), headline.subtext && (_jsx(Typography, { sx: { fontSize: 12, fontWeight: 300, color: 'var(--ink-4)', mt: 0.5 }, children: headline.subtext }))] })), groups.map((group, gi) => (_jsx(Box, { sx: { pb: gi < groups.length - 1 ? 1 : 0, mb: gi < groups.length - 1 ? 1 : 0, borderBottom: gi < groups.length - 1 ? '1px solid var(--rule)' : 'none' }, children: group.rows.map((row) => _jsx(PulseCardRow, { row: row }, row.id)) }, group.key ?? `group-${gi}`))), footerNote && (_jsx(Box, { sx: { pt: 1.5, mt: 0.5, borderTop: '1px solid var(--rule)' }, children: _jsx(Typography, { sx: { fontSize: 12.5, fontWeight: 300, color: 'var(--ink-4)' }, children: footerNote }) })), (footerCta || updatedAt) && (_jsxs(Box, { sx: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', pt: 1, mt: 0.5, borderTop: footerCta && updatedAt && !footerNote ? '1px solid var(--rule)' : 'none' }, children: [footerCta && (_jsxs(Box, { component: "button", onClick: footerCta.onClick, sx: {
