...
| FIN-14 | 🔴 OPEN — BLOCKS RELAUNCH | Shopify App Store review 1.1.4 (factual information) audit, 2026-07-24: `margin-snapshot-integrity.worker.ts` (read-only drift detector, by design never auto-corrects) is actively flagging live drift on `order_margin_snapshot` for `shop_id:1, order_id:c7bad89d-c26b-979d-650f-f2fb1f1d097c` — snapshot `gross_revenue:1479.90` vs authoritative live `2365.85`, confirmed continuously drifted every 5-min poll cycle since at least 2026-07-24 12:09 UTC. Not currently a live 1.1.4 violation since Finances/Cashflow/Margin are frozen and hidden from appLayout — no merchant can see it. But it MUST be resolved before this module is republished, or a merchant would see a materially wrong (~37% understated) margin figure on day one. | Fly prod logs, `[MARGIN_SNAPSHOT_INTEGRITY_DRIFT]`, repeated identical output every cycle, 2026-07-24. | Run `rebuild-from-events` (or a scoped single-order recompute, if one exists) for the listed order(s) as part of the pre-relaunch checklist — check for recurrence across ALL drifted orders at that time, not just this one, before flipping the module back on. |
