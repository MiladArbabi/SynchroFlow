...
⚠️ Current webapp UI is order-centric (WEB-PACK-01). Intended production UX is item-centric free-scan (WEB-PACK-02). Blocked on WEB-PICK-UNIT-01 — all upstream workflows (receive → stow → pick) must be on LSU- unit barcodes before pack free-scan is reliable. WM-34 ✅ + WM-38 ✅ cleared. WM-46 ✅ + WEB-RECEIVE-UNIT-01 ✅ cleared. Remaining blockers: WEB-STOW-UNIT-01 → WEB-PICK-UNIT-01.
...
| WEB-PACK-02 | P1      | 📋 PLANNED      | Pack UI redesign — item-centric free-scan. Packer scans any LSU- unit barcode, system resolves to order+line item, shows variant image + order context + sibling item thumbnails. Auto-print invoice+label on first scan per order. Invoice barcode scan confirms shipment. Batch auto-completes. Removes Brief, Summary, Order Complete screens. Blocked on WEB-PICK-UNIT-01 (all upstream workflows must be on LSU- barcodes before pack free-scan is reliable). |
...
| WEB-PICK-UNIT-01 | P1  | 📋 NEXT         | Pick scan surface migrated to accept LSU- barcodes. Unit status updated stowed → picked on confirm. Blocked on WEB-STOW-UNIT-01 ✅. |
