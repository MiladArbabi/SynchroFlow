...
const canonicalIngestionService = new canonical_commerce_ingestion_service_1.CanonicalCommerceIngestionService();
const orderNexusCanonicalIngestionService = new order_nexus_canonical_ingestion_service_1.OrderNexusCanonicalIngestionService();
...
async function processMessage(msg) {
    ...
    try {
        ...
        // 1) Load staged event
        const stagedEvent = await (0, db_js_1.default)('staged_events')
        ...
        const mappingRules = await (0, db_js_1.default)('data_mapping_rules').where({
        ...
        try {
            await canonicalIngestionService.insertCanonicalOrder(stagedEvent.raw_payload);
        }
        catch (e) {
        ...
        try {
            ...
            if (canonicalOrder && canonicalOrder.id && stagedEvent.shop_id) {
                await orderNexusCanonicalIngestionService.enqueueOrderForOrderNexus(stagedEvent.shop_id, canonicalOrder.id);
            }
        }
        catch (e) {
