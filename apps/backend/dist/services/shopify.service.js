...
const performInitialSync = async (accessToken, platformShopName, shopId, integrationId) => {
  ...
  `;
    try {
        ...
        const totalLineItems = (data.orders?.edges || []).reduce((acc, { node }) => {
...
async function syncOrders(trx, shopId, edges) {
    const ordersToInsert = edges.map(({ node }) => {
...
async function syncOrderLineItems(trx, shopId, orderEdges) {
    ...
    if (lineItemsToInsert.length > 0) {
        await trx('order_line_items')
            ...
            .merge();
        console.log(`[ShopifyService] Synced ${lineItemsToInsert.length} line items.`);
    }
}
