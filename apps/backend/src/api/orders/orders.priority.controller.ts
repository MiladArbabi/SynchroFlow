...
export const httpGetPriorityStack = async (
  ...
  try {
    ...
    const rows = await db('order_risk_snapshot as ors')
      /**
     * Schema-aligned selection.
     * Must match base migration 0053_order_risk_snapshot.
     * If migration changes, this query MUST be updated.
     */
    .select(
    ...
    .orderBy([
     ...
     { column: 'ors.evaluated_at', order: 'desc' },
    ]);
