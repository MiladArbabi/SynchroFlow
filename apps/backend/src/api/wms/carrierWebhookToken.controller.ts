...
export const httpCreateCarrierWebhookToken = async (req: Request, res: Response) => {
  try {
    ...
    const [token] = await withTenant(shopId, (trx) => trx('shop_carrier_webhook_tokens')
...
export const httpRotateCarrierWebhookToken = async (req: Request, res: Response) => {
  try {
    ...
    const updated = await withTenant(shopId, (trx) => trx('shop_carrier_webhook_tokens')
...
export const httpRevokeCarrierWebhookToken = async (req: Request, res: Response) => {
  try {
    ...
    const deleted = await withTenant(shopId, (trx) => trx('shop_carrier_webhook_tokens').where({ id, shop_id: shopId }).delete());
...
export const httpSetCarrierWebhookSecret = async (req: Request, res: Response) => {
  try {
    ...
    const updated = await withTenant(shopId, (trx) => trx('shop_carrier_settings')
