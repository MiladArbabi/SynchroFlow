...
export interface UnifiedCustomerProfile {
  unified_customer_id: string; // Our internal ID (hashed email + domain)
  primary_email: string;
  platforms: PlatformCustomer[];
  resolved_identity: {
    name?: string;
    email: string;
    phone?: string;
  ...
  confidence_score: number; // 0-1 how confident we are in the resolution
  resolution_methods: string[]; // ['email_exact_match', 'phone_match', etc.]
}
...
export class CustomerResolutionService {
   ...
   */
  private static generateUnifiedCustomerId(shopId: number, email: string): string {
   ...
   * Resolve the best identity from platform data
   */
  private static async resolveBestIdentity(platformData: PlatformCustomer[]): Promise<UnifiedCustomerProfile['resolved_identity']> {
   ...
   */
  private static hasConflictingPersonalData(platformData: PlatformCustomer[]): boolean {
    if (platformData.length < 2) return false;
    ...
    if (phones.length >= 2 && new Set(phones).size > 1) return true;
  ...
  private static calculateConfidenceScore(
    ...
    if (platformCount >= 3) factors.push(0.3);
    else if (platformCount === 2) factors.push(0.2);
    else if (platformCount === 1) factors.push(0.1); // +0.1
    ...
    else if (uniqueEmails.size === 1) factors.push(0.2); // Single platform with email +0.2
    else if (uniqueEmails.size === platformData.length) factors.push(0.05); // Reduced from 0.1
    else factors.push(0.1); // Reduced from 0.2

    // Factor 3: Name consistency
    const names = platformData.map(p => `${p.first_name} ${p.last_name}`).filter(name => name.trim());
    const uniqueNames = new Set(names);
    if (uniqueNames.size === 1 && names.length > 1) factors.push(0.3); // Multiple platforms same name
    else if (uniqueNames.size === 1) factors.push(0.05); // Single platform with name +0.05 (reduced from 0.1)
    else if (uniqueNames.size > 0) factors.push(0.01); // Reduced from 0.02
   ...
   */
  private static determineResolutionMethods(platformData: PlatformCustomer[]): string[] {
   ...
   */
  private static extractLocationFromMetadata(metadata?: Record<string, any>): string | undefined {
   ...
   */
  static async findCustomersByEmail(shopId: number, email: string): Promise<UnifiedCustomerProfile | null> {
    try {
      // Get customer data from all platforms for this email
      const platformCustomers: PlatformCustomer[] = [];

      // 1. Get Shopify customer
      const shopifyCustomer = await db('customers')
   ...
   */
  static async getShopUnifiedCustomers(shopId: number): Promise<UnifiedCustomerProfile[]> {
    try {
      // Get all Shopify customers for this shop
      const shopifyCustomers = await db('customers')
        .where({ shop_id: shopId })
        .select('email', 'platform_customer_id', 'first_name', 'last_name', 'phone');

      // Group by email and resolve identities
      const emailGroups = new Map<string, PlatformCustomer[]>();
      
      for (const customer of shopifyCustomers) {
        const platformCustomer: PlatformCustomer = {
          platform: 'shopify',
      ...
      const unifiedProfiles: UnifiedCustomerProfile[] = [];
