...
imageAlt: "Shopify inventory count compared against warehouse system count and physical shelf stock in one operational view"
...
<Checklist items="Pick 5-10 SKUs with known or suspected discrepancies — high-velocity items first, they accumulate gaps fastest|Physically count them. Do not sample from memory or trust the last cycle count|Record all three numbers per SKU: shelf, Shopify, warehouse system|Classify each SKU: systems agree but shelf differs (unrecorded movements) — or systems disagree (sync or adjustment problem)|Find the last date the numbers agreed — order history and adjustment logs date-bound the window|Audit the movements inside that window: receives, returns, refunds, transfers, manual adjustments, app activity" />
