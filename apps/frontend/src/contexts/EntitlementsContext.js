...
export const EntitlementsProvider = ({ children, }) => {
    ...
    const lastGoodSnapshotRef = React.useRef(null);
    ...
    useEffect(() => {
        ...
        axiosInstance
            ...
            const nextShopId = typeof payload.shopId === 'number' ? payload.shopId : null;
            if (import.meta.env.DEV) {
            ...
            if (cancelled)
                return;
            if (import.meta.env.DEV) {
                console.warn('[Entitlements] fetch failed – attempting fallback', err?.message);
            }
    ...
    const hasModule = useCallback((moduleId) => modules.includes(moduleId), [modules]);
    // ⚠️ LEGACY — DO NOT USE IN NEW CODE
    const hasFlag = useCallback((flagId) => flags.includes(flagId), [flags]);
    const value = {
        shopId,
        modules,
        flags,
    ...
    return (_jsx(EntitlementsContext.Provider, { value: value, children: children }));
};
...
//# sourceMappingURL=EntitlementsContext.js.map