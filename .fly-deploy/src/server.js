...
const addonPath = process.env.NODE_ENV === 'test'
    ? path_1.default.join(__dirname, '../../../packages/core-engine/build/Release/sf_core.node') // Path for Jest/ts-jest
    : path_1.default.join(__dirname, './sf_core.node'); // Path for the compiled server.js
...
app.get('/v1/forecast/demand/:sku', async (req, res) => {
    try {
        const { sku } = req.params;
        const forecast = await (0, forecasting_service_1.getDemandForecastForSku)(sku);
        res.json(forecast);
    }
    catch (error) {
        if (error instanceof Error) {
...
app.get('/v1/analytics/inventory-value', async (req, res) => {
    try {
        const totalValue = await (0, forecasting_service_1.calculateTotalInventoryValue)();
...
app.post('/v1/simulations/payment-delay', (req, res) => {
    try {
        ...
        const simulated_cash_flow = (0, forecasting_service_1.simulatePaymentDelay)(current_cash_flow, payment_details);
...
if (require.main === module) {
    app.listen(port, "0.0.0.0", () => {
        ...
        catch (err) { // Added ': any' to fix the implicit 'any' error
